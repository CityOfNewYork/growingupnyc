...
class SmsMeGunyc extends ContactMe {
  protected $action = 'SMS_GUNYC';
  ...
  protected $prefix = 'smnyc_twilio_gunyc';

  const POST_TYPE = 'smnyc-sms-gunyc';
  ...
  public function registerPostType() {
    register_post_type(self::POST_TYPE, array(
      'label' => __('SMNYC SMS - Growing Up NYC', 'text_domain'),
      'description' => __('SMS content for Send Me NYC', 'text_domain'),
      'labels' => array(
        'name' => _x('SMNYC SMS - GUNYC', 'Post Type General Name', 'text_domain'),
        'singular_name' => _x('SMNYC SMS - GUNYC', 'Post Type Singular Name', 'text_domain'),
  ...
  protected function send($to, $msg) {
    try {
      $user = get_option('smnyc_twilio_gunyc_user');
      $secret = get_option('smnyc_twilio_gunyc_secret');
      $from = get_option('smnyc_twilio_gunyc_from');

      $user = (!empty($user)) ? $user : SMNYC_TWILIO_GUNYC_USER;
      $secret = (!empty($secret)) ? $secret : SMNYC_TWILIO_GUNYC_SECRET;
      $from = (!empty($from)) ? $from : SMNYC_TWILIO_GUNYC_FROM;
