...
<?php
  ...
  function render( $params ) {
    ...
    foreach( $values as $result ) {
      ...
      $output .= '<li><a href="'.$url.'?'.$facet['name'].'='.$result['facet_value'].'" class="c-list-box__subitem '.$result['facet_value'].'" aria-selected="' . $selected . '" data-value="' . $result['facet_value'] . '">' . $result['facet_display_value'] . '</a></li>';
    }
    $output .= '</ul>';
    $output .= '</div>';
    return $output;
